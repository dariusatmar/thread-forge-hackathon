...
export async function POST(request: NextRequest) {
  try {
    ...
    if (filters?.platform) {
      where.social_media = filters.platform;
    }
    ...
    const postsSummary = posts.map((post, idx) => {
      ...
      return `Post ${idx + 1} (ID: ${post.id}, Platform: ${post.social_media}, Location: ${post.location}, Category: ${post.category}, Time: ${post.timestamp.toLocaleString()}):
