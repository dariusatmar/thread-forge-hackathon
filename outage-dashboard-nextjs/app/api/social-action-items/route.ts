...
export async function POST(request: NextRequest) {
  try {
    ...
    const posts = await prisma.socialMediaData.findMany({
      where: {
        timestamp: { gte: timeFilter }
      },
      orderBy: { timestamp: 'desc' },
      take: 200, // Limit to most recent 200 posts for analysis
    });
    ...
    const postsSummary = posts.map((post, idx) => {
      return `Post ${idx + 1} (ID: ${post.id}, Platform: ${post.social_media}, Location: ${post.location}, Category: ${post.category}, Time: ${post.timestamp.toISOString()}):
