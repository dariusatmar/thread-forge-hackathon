...
export async function POST(request: NextRequest) {
  try {
    ...
    const potentialSocialPosts = await prisma.socialMediaData.findMany({
      where: {
        timestamp: {
          gte: timeFilter,
        },
        ...
        category: {
          in: ['Service Issue', 'Billing Issue', 'Customer Service Complaint'],
        },
      },
      take: 3,
    });
    ...
    if (potentialSocialPosts.length > 0) {
      churnSignals.push({
        ...
        posts: potentialSocialPosts.map(p => ({
          id: p.id,
          username: p.username,
